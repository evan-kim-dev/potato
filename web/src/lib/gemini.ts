...
export async function generateLiveTripPlan(
  ...
  const system = [
    ...
    "요청한 시·군과 출발·도착 가는 길을 먼저 지키세요. 한산 권역은 가산점일 뿐, 다른 권역 코스로 바꾸지 마세요.",
    alongRoute.length >= 1
    ...
    "예: 춘천→홍천→속초처럼 이어지는 동선(OK). 속초→태백→철원처럼 멀리 점프(금지). 하루는 그 시·군과 인접 권역 안에서 가까운 순.",
  ...
  if (resolved.length < 2) {
    ...
    if (fallback.length < 2) {
      throw new Error(
        "AI가 유효한 스팟을 고르지 못했습니다. 시·군 이름(예: 춘천, 속초, 삼척)을 넣어 다시 시도해 주세요."
      );
    }
    parsed = {
      ...parsed,
